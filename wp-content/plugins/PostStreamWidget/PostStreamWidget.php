...
/*
Plugin Name: Post Stream Widget
...
class PostStreamWidget extends WP_Widget {
	function PostStreamWidget() {
		$this->WP_Widget(
			'post-stream-widget',
			'Post Stream Widget',
			array(
				'classname'		=> 'Post Stream Widget',
				'description'	=> 'A widget that pulls a stream of posts and displays them.'
			),
			array(
				'id_base'		=> 'post-stream-widget'
			)
		);
	}
...
function load_stream_widget() {
	register_widget('PostStreamWidget');
}

add_action('widgets_init', 'load_stream_widget');
